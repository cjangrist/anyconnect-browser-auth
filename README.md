...
- Outbound HTTPS and DNS access to pull the image and at runtime.
...
VPN_HEALTHCHECK_INTERVAL=15s
VPN_HEALTHCHECK_TIMEOUT=10s
VPN_HEALTHCHECK_RETRIES=8
VPN_HEALTHCHECK_START_PERIOD=90s
```
...
Compose pulls `ghcr.io/cjangrist/anyconnect-browser-auth:latest`, a public
`linux/amd64` image built from this repository. `pull_policy: always` ensures each
`docker compose up` checks for the current image.
...
period by default, while the internal browser deadline defaults to 180 seconds.
...
| `VPN_HEALTHCHECK_INTERVAL` | No | `15s` | Delay between Compose healthchecks. |
| `VPN_HEALTHCHECK_TIMEOUT` | No | `10s` | Maximum duration of one Compose healthcheck. |
| `VPN_HEALTHCHECK_RETRIES` | No | `8` | Consecutive failed Compose healthchecks before the container is unhealthy. |
| `VPN_HEALTHCHECK_START_PERIOD` | No | `90s` | Startup grace period before Compose counts healthcheck failures. |
...
Docker build context. Compose passes the four `VPN_*` values as runtime environment
...
GitHub Actions publishes the public `linux/amd64` image to
`ghcr.io/cjangrist/anyconnect-browser-auth:latest` for every repository push. Compose
uses that image directly. Build from source only when developing or validating changes.
...
```bash
docker build --no-cache --tag anyconnect-browser-auth:local .
```
...
1. `node --check src/vpn.js` succeeds.
2. `npm test` succeeds.
3. `docker compose config --quiet` succeeds with a private test `.env`.
4. A no-cache image build succeeds.
5. `/app/vpn.js self-test` passes inside the built image.
6. A fresh container completes browser SSO without a pre-existing profile.
7. `tun0` appears in the container.
8. The manual healthcheck passes and both proxy listeners are present.
9. Ports `8080` and `1080` are published on `0.0.0.0`.
10. IPv4 and, when enabled in the container, IPv6 UID-scoped kill-switch chains are installed.
11. Proxy-owned attempts through `eth0` are rejected while client replies still work.
12. Host and container public IPs differ under the full-tunnel model.
13. HTTP and SOCKS5 proxy egress both differ from host egress, and their proxy-UID
    `tun0` counters increase.
14. The contained Chromium egress test differs from host egress and increases the
    selected proxy UID's `tun0` counter.
15. Restarting the container reauthenticates and returns to healthy.
16. Terminating only the OpenConnect child stops both proxies, produces a new PID,
    and recovers the tunnel and listeners.
17. Complete container logs contain no username, password, TOTP seed, rejection, or
